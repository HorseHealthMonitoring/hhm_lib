...
#define DISPLAY_TEMPERATURE (ESOS_USER_FLAG_D) //define a user flag to notify other tasks when the LCD screen is showing the temperature.
#define DISPLAY_TEMPERATURE_ON (esos_IsUserFlagSet(ESOS_USER_FLAG_D)) //define a macro to check if the user flag for the temperature display is set.

//define the different locations where the custom charcters will be hosted.
...
************************************************************************
*/
//a function to convert the mV output from the temperature sensor to celsius.
...
************************************************************************
*/
//creation of the custom characters used to display the thermometer.
...
//an array to host all of the custom characters for selection later in the tasks.
...
{
	//creating variables to host all of the characters being used for the temperature sensor screen.
	...
	//set the custom characters in the LCDs CGRAM addresses.
	ESOS_TASK_BEGIN();
	...
	while(TRUE)
	{
		//wait until SW3 is pressed and the flag to load the temperature sensor display is high.
		...
		//write the celsius value to the screen.
		u8_row = 1;
		u8_column = 0;
		...
		//logic to determine the custom char writes to the thermometer.
		//essentially checks the celsius value and raises or lowers the thermometer.

		//this logic correlates to the lower half of the thermometer.
		...
		//this logic correlates to the upper half of thermometer.
		...
		//this logic fills the entire thermometer if the temperature crosses the maximum possible temperature.
		else
		{
			u8_tempIndex = 7;
			u8_thermRow = 1;
		...
		ESOS_TASK_WAIT_TICKS(100); //to allow the LCD time to update.
	}// end of while
	ESOS_TASK_END();
